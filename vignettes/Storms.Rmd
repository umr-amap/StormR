...
The `Storms()` function allows to extract tropical cyclone track data for a
...
the `season` arguments are not filled then the `Storms()` function extracts all
...
``` {r chunk1}
sds <- defDatabase(verbose=FALSE)
st <- Storms(sds=sds,loi = "Vanuatu", names = "PAM",verbose=0)
```

The `Storms()` function returns a `StormsList` object in which the first slot
...
``` {r chunk5}
pt <- c(166.45,-22.27)
st <- Storms(sds=sds,loi = pt, seasons = c(2015,2021), verbose=0)
```
...
``` {r chunk9}
sp <- eezNC
st <- Storms(sds=sds,loi = eezNC, season=c(2015,2021),verbose=0)
```
