...
The `Storms()` function returns a `StormsList` object in which the first slot `@data` contains a list of `Storm` objects. With the above specification the `StormsList` contains only one `Storm` object corresponding to cyclone PAM and the track data can be obtained using the `getObs()` function as follows:
