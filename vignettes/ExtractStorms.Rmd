...
``` {r chunk7}
plotStorms(st, sds, names = "NIRAN", labels = TRUE, legends = "bottomleft")
```
...
can be obtained using the `getScale()` function as follows:
...
``` {r chunk12}
plotStorms(st, sds, category = c(4, 5), labels = FALSE, legends = "topright")
```
