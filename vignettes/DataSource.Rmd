...
Before running `StormR` functions users have to provide a tropical storm track dataset as a ".nc" (NetCDF) file in which the location and some characteristics of storms are given across their lifespan. This file is used to create a `StormsDataset` object using the `defDatabase` function. By default, the arguments of `defDatabase` function are set up to create a `StormsDataset` object using the fields provided by USA agencies in the IBTrACS database [International Best Track Archive for Climate Stewardship](https://www.ncei.noaa.gov/products/international-best-track-archive) (Knapp et al., 2010). This database provides a fairly comprehensive record of worldwide tropical storms and cyclones with a 3-hours temporal resolution since 1841. Other databases can be used as long as the following fields are provided:
...
| `rmw` | Radius of maximum winds: distance between the centre of the storm and its band of strongest winds | $km$ | 20 | Recommended* |
...
*Providing `rmw` allows better estimates of wind speed and direction. If not provided `rmw` is approximated using an empirical formula derived from Willoughby et al. (2006) : $R_m = 46.4e^{(-0.0155 \times v_m + 0.0169 \times |\phi|)}$. <br />
**The `sshs` field is optional, if not provided this field is automatically filled using the `msw` field.

### Downloading a data source

If not available, a storm track data set has to be downloaded. In the following example we illustrate how a storm track data set can be downloaded on the IBTrACKS website. Different ".nc" (NetCDF) files containing storm track data sets can be downloaded here: <https://www.ncei.noaa.gov/data/international-best-track-archive-for-climate-stewardship-ibtracs/v04r00/access/netcdf/>. Please, follow the IBTrACKS citation recommendations when using these datasets (including citing Knapp et al., 2010 and Knapp et al., 2018, and providing the access date for the later). For example, track data for all the storms that occurred over the last three years over the different basin world wide can be download on the working directory as follows, 
...
Once a storm track data set is available a `StormsDataset` object can be created using the `defDatabase` function as follows,

```{r}
sds <- defDatabase(filename = "IBTrACS_ALL_3yrs.nc", basin=NULL,verbose = FALSE)
str(sds)
length(unique(sds@database$names))
```

This `StormsDataset` object contains track data for 270 storms. When using an IBTrACKS storm track data set, default arguments can be used. By default, field provided by USA agencies (starting with "usa_") are used, but other fields can be selected. For example, for the South Pacific basin ("SP"), we can choose to use data provided by the Australian bureau of meteorology (starting with "bom_") as follows,
...
This `StormsDataset` object contains track data for 27 storms in the South Pacific basin.

### Unit conversion

In the IBTrACKS data sets speeds are given in knots ($knt$), distances in nautical miles ($nm$), and pressure in millibar ($mb$) but the models implemented in the `temporalBehaviour` and `spatialBehaviour` functions to compute wind speed, wind direction, and summary statistics require data following the international system of units (SI) or SI-derived units. Therefore, by default, when using IBTrACKS data sets, the `defDatabase` function converts knots ($knt$) to meter per second ($m.s^{-1}$), nautical miles ($nm$) to kilometres ($km$), and millibar ($mb$) to Pascal ($pa$). When using another source of data than IBTrACKS, the `unit_conversion` argument can be used to convert the data to the desire units. 
...
It is important to pay attention to the formats and units of data provided by the database. If `msw`, `rmw`, `pressure` and/or `poci` fields units do not match the required units a conversion will have to be performed during the creation of `StormsDataset` object.
...
In the following example, we will use the data from the IBTrACS that gather all storms that occurred in the 
