...
Two `test_dataset` are provided with the `StormR` package (a NetCDF and a CSV file). These test data sets
...
In addition to these mandatory rules above, the default separator is set to a single comma ','. Nevertheless th user might decide to change it using the `sep` parameter in `defStormsDataset()`.
