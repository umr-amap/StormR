...
test_that("Test computeAsymetry", {
  res <- computeAsymmetry(
    asymmetry = "Chen",
    speed = c(10, 12),
    x = c(1, 2),
    y = c(1, 2),
  ...
  expect_named(res, c("speed", "direction"))
  expect_type(res$speed, "double")
  expect_type(res$direction, "double")
  ...
  expect_length(res$speed, 3)
  expect_length(res$direction, 3)

  expect_true(all(res$direction >= 0))
  expect_true(all(res$direction <= 360))
  ...
  res_north <- do.call(computeAsymmetry, c(args, list(lat = 20)))
  res_south <- do.call(computeAsymmetry, c(args, list(lat = -20)))

  expect_type(res_north$direction, "double")
  expect_type(res_south$direction, "double")
})
...
test_that("Test computeDirection function with land-interaction for Boose model", {
  expect_equal(computeDirection(1, 1, 30, 1), 95)
  expect_equal(computeDirection(1, 1, 30, 0), 115)
  expect_equal(computeDirection(1, -1, 30, 1), 185)
  expect_equal(computeDirection(1, -1, 30, 0), 205)
  expect_equal(computeDirection(-1, 1, 30, 1), 5)
  expect_equal(computeDirection(-1, 1, 30, 0), 25)

  expect_equal(computeDirection(1, 1, -30, 1), 355)
  expect_equal(computeDirection(1, 1, -30, 0), 335)
  expect_equal(computeDirection(-1, -1, -30, 1), 175)
  expect_equal(computeDirection(-1, -1, -30, 0), 155)
})
