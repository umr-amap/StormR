...
``` r
st <- getStorms(loi = "Vanuatu", names = "PAM")
st_prod <- stormBehaviour_sp(st, product = c("MSW", "PDI", "Exposure"),wind_threshold=49)
plotBehaviour(st, st_prod[["PAM_MSW"]])
plotBehaviour(st, st_prod[["PAM_PDI"]])
plotBehaviour(st, st_prod[["PAM_Exposure_49"]])
```
